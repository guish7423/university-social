  ...
  <div class="friends-page">
    ...
    <el-tabs v-model="tab">
      ...
      <el-tab-pane label="好友请求" name="requests">
        ...
        <div v-else class="user-list">
          <div v-for="r in requests" class="user-row stagger-item" :key="r.id">
            <el-avatar :size="40" :src="r.avatar" />
            <div class="user-info">
              <div class="name">{{ r.nickname }}</div>
            </div>
            <div class="actions">
              <el-button type="primary" size="small" @click="handleAccept(r.id)">接受</el-button>
...
async function handleAccept(id: number) {
  try { await acceptFriendRequest(id); requests.value = requests.value.filter(r => r.id !== id) }
  catch { /* handled by interceptor */ }
}

async function handleReject(id: number) {
  try { await rejectFriendRequest(id); requests.value = requests.value.filter(r => r.id !== id) }
  catch { /* handled by interceptor */ }
}
...
.user-row {
  ...
  border-radius: $radius-md; padding: 12px; cursor: pointer; transition: all 0.2s;
  &:hover { border-color: $primary-light; }
  .user-info { flex: 1; min-width: 0;
    .name { font-size: 14px; font-weight: 600; }
    .school { font-size: 12px; color: $text-muted; }
  }
